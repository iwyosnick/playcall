...
const EditableHeader: React.FC<{
    ...
    const handleCommit = () => {
        if (label.trim()) {
            ...
            // Revert on empty, which will commit the original label and end editing
    ...
    const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
        if (e.key === 'Enter') {
            ...
            setLabel(header.label); // Revert local state
            onStartEdit(null); // End editing without committing
        }
    }
    ...
    <div className="overflow-x-auto rounded-lg border border-gray-700">
      <table className="min-w-full divide-y divide-gray-700">
        ...
        <tbody className="divide-y divide-gray-800 bg-gray-900/50">
          {players.map((player) => (
            <tr key={player.name} className="hover:bg-gray-700/50 transition-colors group">
              {headers.map(header => {
                  const isSticky = header.key === 'snakeRank' || header.key === 'name';
                  const stickyClass = 
                  ...
                  return (
                    <td 
                        key={header.key}
                        className={`whitespace-nowrap px-4 py-4 text-sm text-gray-400 bg-inherit group-hover:bg-gray-700/50 ${stickyClass}
